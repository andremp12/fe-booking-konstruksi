...
export default defineComponent({
  components: {FormSatuan, AppBreadcrumb},
  computed: {
    appMixin() {
      return appMixin
    }
            ...
            <div class="w-full p-4 border-2 border-gray-200 rounded-lg h-fit">
              <div class="w-full space-y-2 font-semibold text-gray-500 text-sm">
                ...
                <div class="w-full flex space-x-2 justify-between items-start">
                  ...
                  <label v-if="data.status != 'Booking' && data.status != 'Canceled'" class="block text-gray-800 font-mono mb-2 w-2/3 text-end">{{ data.remaining_payment ? 'RP.'+appMixin.accountingToPrice(data.remaining_payment) : '-' }}</label>
